...
public class GlideUtils implements cn.finalteam.galleryfinal.ImageLoader{
     ...
     * 高效缓存策略    支持Memory和Disk图片缓存 默认Bitmap`格式采用RGB_565内存使用至少减少一半
    ...
    public static void  displayImageView(ImageView mImageView,String imageUrl){
        Glide.with(APP.the()).load(imageUrl).transform(new GlideCircleTransform(mImageView.getContext())).into(mImageView);
    }
    ...
    public static void displayCircleImageView(ImageView mImageView,String imageUrl,Drawable error){
        Glide.with(APP.the()).load(imageUrl).error(error).transform(new GlideCircleTransform(mImageView.getContext())).into(mImageView);
    }
