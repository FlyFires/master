...
public class WindowUtils {
	...
	//app恢复到前台
	public static void moveToFront(Context mContext) {
		...
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
			...
			for (ActivityManager.AppTask appTask : list) {
				if (appTask.getTaskInfo().id == processID) {
//					am.moveTaskToFront(processID, ActivityManager.MOVE_TASK_NO_USER_ACTION);
					appTask.moveToFront();
					break;
				}
			}
		...
		//某些机型需要添加以下代码才能正常恢复到前台
		SystemClock.sleep(100);
		Activity activity = WindowUtils.getActivity();
		if (activity == null) {
			Intent intent = new Intent(mContext, WaitCallActivity.class);
	...
	public static Activity getTopActivity() {
		APP application = getMediaSDKApplication();
		if (application != null) {
	 ...
	 * @return
	 */
	public static APP getMediaSDKApplication() {
		...
		try {
			...
			return (APP) method1.invoke(object, (Object[]) null);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
