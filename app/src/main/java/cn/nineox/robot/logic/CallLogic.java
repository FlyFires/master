    ...
    public void receiveIOCtrlDataInfo(int sid, int avIOCtrlMsgType, byte[] data) {
        ...
        switch (avIOCtrlMsgType) {
            case CustomCommand.IOTYPE_USER_IPCAM_CALL_END: {
                Log.e(TAG, "CallLogic2 receiveIOCtrlDataInfo:" + avIOCtrlMsgType + mAccountID);
...
//       rtt = st_avStatus.RoundTripTime;
    ...
    public void switchAudioMode() {
        ...
        mDataBinding.decodePreviewBig.setVisibility(View.INVISIBLE);
        mDataBinding.btnLayout.setVisibility(View.VISIBLE);
        ConnectInfo accountInfo = App.sConnectList.get(0);
        //停止视频编码
        ...
        mDataBinding.videoBtnview.setVisibility(View.GONE);
        mDataBinding.audioBtnview.setVisibility(View.VISIBLE);
        if (accountInfo.mByClientConnect) {
            TUTKP2P.TK_getInstance().TK_device_sendIOCtrl(
                    accountInfo.mClientSID,
                    CustomCommand.IOTYPE_USER_IPCAM_SWITCH_AUDIO,
...
//        if (mDeviceBean != null) {
//            mLogic.saveVideoRecord(mDeviceBean.getMid(), mIsHost ? 1 : 0, mStartTime, System.currentTimeMillis());
//        }
...
//        mView.setItemNickName(pos, bigInfo.mNickName);
    ...
    private OnMediaListener mOnMediaListener = new OnMediaListener() {
        ...
        public void onVideoEncodeCallback(byte[] data, int width, int height, boolean isIFrame, long timeStamp) {
            ...
            Log.e("--","onVideoEncodeCallback:" + this   + "   " + App.sConnectList.size());
...
//            }
    ...
    public void receiveVideoInfo(int sid, byte[] videoData, long timeStamp, int number, int onlineNum, boolean isIFrame) {
        ...
        android.util.Log.e("--","receiveVideoInfo:" + this + "   " + App.sConnectList.size());
...
//        }
    ...
    public void receiveIOCtrlDataInfo(String uid, int avChannel, int avIOCtrlMsgType, byte[] data) {
        ...
        Log.e(TAG, "CallLogic receiveIOCtrlDataInfo:" + avIOCtrlMsgType + mAccountID);
        switch (avIOCtrlMsgType) {
...
//                    ((Activity) mActivity).setResult(RESULT_OK);
    ...
    public void saveVideoRecord(String mid, int type, long startTime, long endTime) {
        StringReqeust reqeust = new StringReqeust(Const.URL_VIDEORECORD_SAVE);
        reqeust.add("mid", mid);
