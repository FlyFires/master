...
public abstract class AbstractRequest<T> extends RestRequest<Result<T>> {
    ...
    public Result<T> parseResponse(Headers headers, byte[] body) throws Exception {
        ...
        if (responseCode == 200 && body != null && body.length > 0) {
            ...
            try {
                ...
                String errorCode = ob.optString("errorCode", "");
                String eventList = ob.optString("eventList", "");
                Log.i("HTTP response>>>" + "status:" + status);
                if ("ERROR_CODE_INVALID_TOKEN".equals(status)) {
                    Intent intent = new Intent();
                    intent.setAction("ERROR_CODE_INVALID_TOKEN");
                    NoHttp.getContext().sendBroadcast(intent);
                }
                if (!TextUtils.isEmpty(eventList) && eventList.length() > 0 && !"null".equals(eventList)) {
                    Intent intent = new Intent();
                    intent.setAction("USER_CHANGE");
                    intent.putExtra("eventList",eventList);
                    NoHttp.getContext().sendBroadcast(intent);
                    Log.e("--","sendBroadcast:USER_CHANGE");
                }
                //这样处理是为了去掉returndata这一层
                if ("success".equals(status) || "success".equals(errorCode)) {
                    String value = ob.optString("data", "");
                    T t = getResult(value);
                    return new Result<>(200, t, headers, message);
                } else {
                    return new Result<>(responseCode, null, headers, message);
                }
    ...
    public void excute(ResponseListener<T> listener){
        NoHttp.getRequestQueueInstance().add(0, this, new DefaultResponseListener<T>(this,listener));
    }
}