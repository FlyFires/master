...
public class SilentInstaller {
    ...
    public int installSilent(Context context, String filePath) {
        return installSilent(context, filePath, " -r " + getInstallLocationParams(context));
    }
    ...
    public int installSilent(Context context, String filePath, String pmParams) {
        ...
        //设置默认安装位置
//        StringBuilder command2 = new StringBuilder().append("LD_LIBRARY_PATH=/vendor/lib:/system/lib pm set-install-location 2");
//        ShellUtil.CommandResult commandResult2 = ShellUtil.execCommand(command2.toString(),
//                !isSystemApplication(context), true);
        ...
        //Log.e("SilentInstaller","command2:" + command2.toString());

        Log.e("SilentInstaller","command:" + command.toString());
        if (commandResult.responseMsg != null
                && (commandResult.responseMsg.contains("Success") || commandResult.responseMsg.contains("success"))) {

            Intent intent = new Intent("cn.nineox.robot.appstrore.SilentInstaller");
            PackageInfo info = context.getPackageManager().getPackageArchiveInfo(file.getAbsolutePath(), PackageManager.GET_ACTIVITIES);
            if(info != null){
     ...
     */
    public int getInstallLocation(Context context) {
...
//        }
     ...
     * @return
     */
    private String getInstallLocationParams(Context context) {
        int location = getInstallLocation(context);
        switch (location) {
        ...
        Log.e("SilentInstaller","getInstallLocationParams:" + location);
        return "";
    }
